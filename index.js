...
console.log('task 1b', createMenuItem('burritos', 10, 'Dinner'));
console.log('task 1b', createMenuItem('Chicken', 10, 'Lunch'));
console.log('task 1b', createMenuItem('Pizza', 15, 'Dinner'));
console.log('task 1b', createMenuItem('Burger', 9, 'Lunch'));
...
*/
reviews.push({name: "Eric", rating: 10, feedback: "Eric is the best"})
...
function getReviewByIndex(a, n) {
  return `${a[n].name} gave the restaurant a ${a[n].rating} star review, and their feedback was: ${a[n].feedback}`
}
...
function getReviewByRating(a , n) {
  let newA = [];
...
function getLongReviews(a) {
  let newA = [];
  for(let i = 0; i < a.length; i++){
    if(a[i]['feedback'].split(" ").length > 15){
      newA.push(a[i])
    }
  }
  return newA
